...
makeButton("⚡ Jump + Block Menu", function()
    ...
    subGui = Instance.new("ScreenGui", player.PlayerGui)
    subGui.Name = "MCJ_SubMenu"
    ...
    -- 🟢 Jump Boost
    local boostOn = false
    local jumpBtn = subButton("🟢 Jump Boost", 40, function()
    ...
    local blockOn = false
    local megaBlock
    local blockBtn = subButton("🟦 Mega Block", 90, function()
        blockOn = not blockOn
        if blockOn then
            megaBlock = Instance.new("Part")
            megaBlock.Size = Vector3.new(10000,1,10000)
            megaBlock.Anchored = true
            megaBlock.Position = player.Character.HumanoidRootPart.Position - Vector3.new(0,5,0)
            megaBlock.Color = Color3.fromRGB(100,100,255)
            megaBlock.Transparency = 0.2
